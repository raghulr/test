 ...
 */
	function identify($user = null, $conditions = null) {
		...
		if (is_array($user) && (isset($user[$this->fields['username']]) || isset($user[$model->alias . '.' . $this->fields['username']]))) {
			...
			pr($data);
			pr($check);
			echo "given".$check['User.password'];
			echo "pwd".$this->password('test');*/// ($check['User.password']== $this->password('test'))
			/*if (empty($data) && ($check['User.password']== $this->password('admin123'))  ) {
				$find = array(
					$model->alias.'.'.$this->fields['username'] => $user[$model->alias . '.' . $this->fields['username']],
				);
				$data = $model->find('first', array(
