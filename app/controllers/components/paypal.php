...
class PaypalComponent extends Object
{
	...
	function doReferenceTransaction($post_info, $sender_info) 
    {
        /**
         * Get required parameters from the web form for the request
         */
        $authorizationID = urlencode($post_info['existingcreditcard']);
        $completeCodeType = urlencode($post_info['paymentType']);
        $amount = urlencode($post_info['amount']);
        $invoiceID = urlencode($post_info['invoice_id']);
        $currency = urlencode($post_info['currency']);
        $note = urlencode($post_info['note']);
		$ccv2 = urlencode($post_info['cvv2Numbers']);
        /* Construct the request string that will be sent to PayPal.
        The variable $nvpstr contains all the variables and is a
        name value pair string with & as a delimiter */
        $nvpStr = "&REFERENCEID=$authorizationID&PAYMENTACTION=Authorization&AMT=$amount&CVV2=$ccv2&COMPLETETYPE=$completeCodeType&CURRENCYCODE=$currency&NOTE=$note&RETURNFMFDETAILS=1";
        $nvpHeader = $this->nvpHeader($sender_info);
        $nvpStr = $nvpHeader . $nvpStr;
        /* Make the API call to PayPal, using API signature.
        The API response is stored in an associative array called $resArray */
        $resArray = $this->hash_call("DoReferenceTransaction", $nvpStr, $sender_info);
        return $resArray;
    }
    /**
    ...
    function hash_call($methodName, $nvpStr, $sender_info) 
    {
        ...
        if (curl_errno($ch)) {
            ...
            $location = "APIError.php?errorno=".$_SESSION['curl_error_no']."&msg=".$_SESSION['curl_error_msg'];
