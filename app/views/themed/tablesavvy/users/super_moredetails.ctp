...
ul.dash-list span.dash-button a{-webkit-border-radius: 5px; -moz-border-radius: 5px; border-radius: 5px;
background-color:#2d73e2; border:1px solid #6ea0de; display:block;
background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#82bff5), to(#2d73e2));
  ...
  background: -o-linear-gradient(top, #82bff5, #2d73e2); float:left; padding:0 20px; color:#fff; text-decoration:none;}
 ul.dash-list span.dash-button a:hover{
...
<div class="dash-wrap">
    ...
    <div class="dash-bottom">
        <div>
            <?php if(!empty($rest)){
                    $seating_cus = $rest['Offer']['seating_custom'];
                    $seating = $rest['Offer']['seating'];
            ?>
            <ul class="dash-list02">
                <li>
                    <strong>
                        <?php 
                            $user_fname= $rest['User']['firstName'];
                            $user_lname= $rest['User']['lastName'];
                            if(!empty($user_fname)) echo $user_fname.' ';
                            if(!empty($user_lname)) echo $user_lname; 
                        ?>
                    </strong> 
                </li>
                <li>Transaction Date/Time: <?php echo $rest['Reservation']['trasaction_time'];?></li>
                <?php 
                    $rest_id = $rest['Offer']['restaurantId'];
                    $row=mysql_fetch_array(mysql_query("select restaurants.name,restaurants.phone from restaurants where restaurants.id='".$rest_id."' "));
                    if(strlen($row['name'])>20)
                        $rest_name=substr($row['name'],0,18 ).'...';
                     else
                        $rest_name= $row['name'];
                ?>
                <li>Restaurant: <?php echo $rest_name?></li>
                <li>Reservation Time: <?php echo date('g:i A',strtotime($rest['Offer']['offerTime']));?></li>
                <li>Party Size:                     
                    <?php 
                        if(!empty($seating)){ 
                            if($seating_cus!=0)
                                echo $seating_cus;
                            elseif($rest['Reservation']['cancel_custom']!=0)
                                echo $rest['Reservation']['cancel_custom'];
                            else
                                echo $seating; 
                        }
                    ?></li>
                ...
                <li>Phone Number: 
                    <?php if(!empty($rest['User']['phone'])){
                                $phone = str_replace(array(' ', '<', '>', '&','+', '{', '}', '*','-',')','('), array(''), $rest['User']['phone']);
                                if(  preg_match( '/^(\d{3})(\d{3})(\d{4})$/', $phone,  $matches ) ){
                                    $result = '('.$matches[1].')' . '-' .$matches[2] . '-' . $matches[3];
                                    $user_call=$matches[1].$matches[2] .$matches[3];
                                }else
                                    $result='Nil';
                    }?>
                    <a href="tel:<?php echo $user_call;?>">
                        <?php echo $result;?>
                    </a>
                </li>
            </ul>
