<?php 
	echo $javascript->link('libs/jquery.jcarousel', false);
...
.expired_count .countdown_section{ 
	float:left; 
	color:#000000; 
	overflow:hidden; 
	width:70px; 
...
jQuery(document).ready(function() {
	...
	var end_date=parseInt($('.js-time1').html());
	parent.Cufon.replace('#numbers2',{ fontFamily: 'bebas' });
	$("#numbers2").countdown({until:end_date,format:'HMS',onTick:timecufon,tickInterval:60});
	setreservationTime();
	...
	document.getElementById('HomeSearch').disabled = true;
});
...
<?php echo $form->create('Home', array('action' => 'index'));?>    
	<div class="prodcucontainer" id="select_search1">
    	<div class="expire_time">
        	 <div style="font-family:Arial; font-weight:bold; font-size:17px; color:#FFFFFF; margin:36px 0 0 104px;">
             	HANG TIGHT EARLY BIRD
             </div>
             <div style="font-family:Arial; font-weight:bold; font-size:23px; color:#FFFFFF; margin:10px 0 0 40px;">
             	deals go live tomorrow morning
             </div>
             <div style="font-family:Arial; font-weight:bold; font-size:14px; color:#FFFFFF; margin:20px 0 0 76px;">
             	check out some of today's participating
             </div>
             <div style="font-family:Arial; font-weight:bold; font-size:14px; color:#FFFFFF; margin:20px 0 0 142px;">
             	restaurants below...
             </div>
             <div class="js-time1 hide" style="display:none;"><?php  $dealtime='11:00:00';  echo $end_time = intval(time()- strtotime(date('Y-m-d').' '.$dealtime)); ?></div>
			 <div id="numbers2" class="expired_count"></div>
        </div>
        <div class="expire_howitworks">
		
        </div>
    </div>       
    <div class="prodcucontainer" id="select_search">
        <ul id="rest_product" class="rest_product">
         <div class="bordersearch"><?php  echo "want to check out our restaurants? "; ?></div>
            ...
            <li class="product" id="prodcucontainer_<?php echo $data['Restaurant']['id']; ?>">
               ...
               <?php $url = $html->url(array('controller'=>'homes', 'action'=>'details'),true);?>
                ...
                <span class="product-name">
                 <?php $url = $html->url(array('controller'=>'homes', 'action'=>'details'),true);?>
                	<a class="res_name" href="javascript:;">
				...
				<?php
					...
					for($i=0;$i<=$diff;$i++){
					//$party_size;
					$resservation = $restaurant->Restaurant->tableAvailability_expired($data['Restaurant']['id'],$starttime,$party_size);
					//pr($resservation);
				...
				<li class="<?php if($i==0) echo "time1"; else if($i == $diff) echo "time5"; else  echo "time3";  ?>">
					...
					<a class="changehover" id="time_<?php echo $data['Restaurant']['id'].'_'.$starttime;?>" onmouseover="mouseover('<?php echo $starttime; ?>','<?php echo $data['Restaurant']['id']; ?>')" onmouseout="mouseout('<?php echo $starttime; ?>','<?php echo $data['Restaurant']['id']; ?>')" href="javascript:;" ><?php echo date('h:i A',$starttime); ?></a>     
				</li>
