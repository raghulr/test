...
<div id="changedate" style="position:relative;">
<div id="gif" style="display:none;">
	 <?php echo $html->image('/images/wait.gif', array('alt'=>'wait','border'=>0));?>
</div>
<?php
	//if(isset($neigh_name)){ 
		$changedateurl=$html->url(array('controller'=>'table','action'=>'changedate'),true); ?>
...
<div class="date_set1">Select date for future reservation</div>
	<?php //} ?>
    <div class="cont_navi">
        <ul class="navigation">
    ...
    <div class="navi_content">
    	<div class="table_content" id="table_content">
            ...
            <div class="drop-box">
            <div class="tableres"> 
            	<div class="table_available">Table Availability for</div>
            	<input id="rdate"  value="<?php echo "today"; ?>" name="date1" disabled="disabled">
            </div>
            <?php if(isset($starttime)&&$interval!=0){?>
    	    <a href="<?php echo $html->url(array('controller'=>'table','action'=>'recurringtable'),true).'?date='.date('Y-m-d'); ?>" class="colorbox-addrec"><?php echo $html->image('/images/addrecurr.png',array('border'=>0,'width'=>174,'height'=>30)); ?></a>
             <?php $time_url = $html->url(array('controller'=>'table','action'=>'cleartable',date('Y-m-d')),true); ?>
            <a href="javascript:;" onclick="return sample('<?php echo $time_url; ?>');">
              <?php echo $html->image('/images/clear_all_new.png',array('border'=>0,'width'=>90,'height'=>30)); ?>
            </a>
            <?php } ?>
           <?php /*?> <a href="<?php echo $html->url(array('controller'=>'table','action'=>'cleartable',date('Y-m-d')),true); ?>"><?php echo $html->image('/images/clear_all.png',array('border'=>0,'width'=>173,'height'=>30)); ?></a><?php */?>
    	    </div>
		...
		if($interval!=0 && !empty($interval)){
			$grandSum =0;
                        $newoffer = '';
                        $newpurchased='';
                        $reservation = $tables->getRestaurantReservation(date('Y-m-d'));
                        $offer = $tables->getAvilableTables($res_id, $reservation, date('Y-m-d')); //pr($offer);
                        $purchased = $tables->getPurchased($res_id, $reservation, date('Y-m-d')); //pr($purchased);
			...
			for($i=0;$i<=$interval;$i++){ 
				...
				?>
                 <fieldset id="update_clear" class="field" id="<?php echo 'int_'.$i;?>">
                    ...
                    <ul id="table_list">
                    <?php for($k=0;$k<4;$k++){ ?>
                        <li onMouseOver="show('<?php echo date('H:i:s',$starttime); ?>','<?php echo $timearray[$k]; ?>','<?php echo $i;?>','<?php echo date('Y-m-d'); ?>')" onMouseOut="hide('<?php echo $timearray[$k]; ?>','<?php echo $i;?>')">
                          <?php //$offer=$tables->getTables(date('H:i:s',$starttime),date('Y-m-d'),$timearray[$k]);?>
                          <?php //$offer_pur=$tables->getPur(date('H:i:s',$starttime),date('Y-m-d'),$timearray[$k]);
						//pr($offer_pur['Offer']['id']);
						  ?>
                            <a href="javascript:;" >
                                            <span class="set_back">
...
<script type="text/javascript">
	var timer =null;
	timer=setTimeout(function() { changedate('<?php echo date('m/d/Y'); ?>','<?php  echo $changedateurl; ?>'); },180000);
</script>