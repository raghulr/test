                 ...
                 <fieldset class="field" id="<?php echo 'int_'.$i;?>">
                    ...
                    <?php for($k=0;$k<4;$k++){  ?>
                        <li onMouseOver="show('<?php echo date('H:i:s',$starttime); ?>','<?php echo $timearray[$k]; ?>','<?php echo $i;?>','<?php echo $date2; ?>')" onMouseOut="hide('<?php echo $timearray[$k]; ?>','<?php echo $i;?>')">
                          <?php $offer=$tables->getTables(date('H:i:s',$starttime),$date2,$timearray[$k]);?>
                          <?php $offer_pur=$tables->getPur(date('H:i:s',$starttime),$date2,$timearray[$k]);
						  ?>
                            <a href="javascript:;" onMouseOver="show('<?php echo date('H:i:s',$starttime); ?>','<?php echo $timearray[$k]; ?>','<?php echo $i;?>','<?php echo $date2; ?>')" onMouseOut="hide('<?php echo $timearray[$k]; ?>','<?php echo $i;?>')">
                                <span class="set_back"><div class="text" id="<?php echo 'int_'.$i.'_'.$timearray[$k];?>"><?php if(!empty($offer[0]['total'])){ $sum+=$offer[0]['total']; echo $offer[0]['total']; $grandSum = $grandSum+$offer[0]['total'];}?></div></span>
                                  <span class="set_back1"><div  class="text" id="<?php echo 'int_'.$i.'_'.$timearray[$k];?>"><?php if(!empty($offer_pur[0]['total'])){ echo $offer_pur[0]['total'];}?></div></span>
                    	...
                    	<div id="<?php echo 'total_'.$i; ?>" style="float:left;margin-top:30px;margin-left:0px; font-family:Arial; font-size:24px; color:#595959;">
                    <?php echo $sum; ?>
                    </div>
                </fieldset>
