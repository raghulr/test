...
<div class="table_container">
	<div class="drop-down">
        ...
        <p>Table Avaliablity for <?php echo date('m/d/Y'); ?></p>
        </div>
  		<div class="drop-box">
    	    <a href="<?php echo $html->url(array('controller'=>'table','action'=>'recurringtable'),true).'?date='.date('Y-m-d'); ?>" class="colorbox-min">Add Recurring table</a>
    	</div>
    </div>
                    ...
                    <?php for($k=0;$k<4;$k++){ ?>
                        <li>
                          <?php $offer=$tables->getTables(date('H:i:s',$starttime),date('Y-m-d'),$timearray[$k]);?>
                            <a href="javascript:;" onMouseOver="show('<?php echo date('H:i:s',$starttime); ?>','<?php echo $timearray[$k]; ?>','<?php echo $i;?>','<?php echo date('Y-m-d'); ?>')" onMouseOut="hide('<?php echo $timearray[$k]; ?>','<?php echo $i;?>')">
